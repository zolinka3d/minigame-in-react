...
const blocksPositions = [
  blocksValues.blok1.position,
  blocksValues.blok2.position,
  blocksValues.blok3.position,
  blocksValues.blok4.position,
];
...
function App() {
  const {
    ...
    score,
    setScore,
    positionOfCorrectBlock,
    setPositionOfCorrectBlock,
  ...
  const checkingCorrectPosition = (positionOfCorrectBlock) => {
    if (
      manPosition + 50 > positionOfCorrectBlock &&
      positionOfCorrectBlock + 30 > manPosition
    )
      return true;
    else return false;
  };
  ...
  useEffect(() => {
    ...
    if (
      blockPosition > GameBoxValues.height - 1 &&
      checkingCorrectPosition(positionOfCorrectBlock) === true
    ) {
      //console.log(score)
      ...
      blockPosition > GameBoxValues.height - 1 &&
      checkingCorrectPosition(positionOfCorrectBlock) === false
    ) {
      setScore((score) => score - 1);
    }
    ...
    <>
      <div ref={ref} onKeyDown={handleKeyDown} tabIndex={-1} className="App">
        <GameBox height={GameBoxValues.height} width={GameBoxValues.width}>
          <Block
            left={blocksValues.blok1.position}
            width={BlockValues.width}
            top={blockPosition}
            boolean={blok1}
          />
          <Block
            left={blocksValues.blok2.position}
            width={BlockValues.width}
            top={blockPosition}
            boolean={blok2}
          />
          <Block
            left={blocksValues.blok3.position}
            width={BlockValues.width}
            top={blockPosition}
            boolean={blok3}
          />
          <Block
            left={blocksValues.blok4.position}
            width={BlockValues.width}
            top={blockPosition}
            boolean={blok4}
          />
          <Man
            src={ziomek}
            top={GameBoxValues.height - ManValues.height}
            width={ManValues.fullwidth}
            left={manPosition}
            direction={manDirection}
          />
        </GameBox>
      </div>
